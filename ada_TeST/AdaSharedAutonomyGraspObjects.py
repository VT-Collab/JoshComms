...
def Init_Goals(env, robot, randomize_goal_init=False):
    ...
    goal_objects.append(env.fork_details)
...
def goal_from_object(env,obj):
 ...
 # print(type(object))
  ...
  num_sampled = 0
  manip = env.panda
  obj_pos = obj['positions']
  obj_quat = obj['quats']
  #print(pose)
  for i in range(len(obj['grasp'])):
    pos = obj_pos[i]
    #print(pos)
    quat = obj_quat[i]
  ...
  #print("ik_sol")
  #print(ik_sol)
  return Goal(obj_quat,obj_pos ,grasp=obj['grasp'], target_poses = target_poses, target_iks = target_iks)
...
if __name__ == "__main__":
  ...
  for i in range(1):
    ...
    ada_handler.execute_policy_sim(direct_teleop_only=True, fix_magnitude_user_command=False)
