...
def getState(x):
    curr_pos_awrap = convert_to_6d(x["curr_pos"])
    z = list(x["curr_q"])
    z.extend(curr_pos_awrap)
    z.append(x["curr_gripper_pos"])
    z.append(float(x["trans_mode"]))
    z.append(float(x["slow_mode"]))
    
    # z = np.array([x["curr_q"], curr_pos_awrap, x["curr_gripper_pos"], x["trans_mode"], x["slow_mode"]])
    # z = x["curr_q"] + curr_pos_awrap + x["curr_gripper_pos"] + x["trans_mode"] + x["slow_mode"]
...
def noiseData(x, nextX, noiselevel=0.0005, mover=None):
    curr_pos = np.asarray(x["curr_pos"])
    curr_q = np.asarray(x["curr_q"])
    curr_gripper_pos = x["curr_gripper_pos"]
    curr_trans_mode = x["trans_mode"]
    curr_slow_mode = x["slow_mode"]
    ...
    state = noise_q.tolist() + noise_pos_awrap.tolist()
    state.append(float(curr_gripper_pos)) 
    state.append(float(curr_trans_mode))
    state.append(float(curr_slow_mode))
    return state, action


if __name__ == "__main__":
    rospy.init_node("sample") # this is bullshit
    strs = ["intent1", "intent2", "intent0"]
    strs = ["../ros-gcl/intent_samples/" + s for s in strs]
    ...
    for s in strs:
        ...
        for f in files:
            ...
            for i in range(len(data) - lookahead - 1):
                ...
                for _ in range(noisesamples):
                    state, action = noiseData(x, data[i+lookahead], mover=mover)
                    if state is None: 
                        continue
                    newdata.append((state, action, [0], getState(data[i+1]), [0]))
                # newdata.append((state, action, [0], getState(data[i+1]), [0]))/
        ...
        fname = "{}/all".format(s)
        pickle.dump(newdata, open(fname+"_sac_new.pkl", "wb"))
