...
from utils import TrajectoryClient, RemoteClient, convert_to_6d
...
def run_test(args):
    ...
    while not rospy.is_shutdown():
        ...
        axes, gripper, mode, slow, start, assist_in = remote.getInput()#joystick.getInput()
        # print(axes)
        # remote.print_msg
        ...
        xdot_h = np.zeros(6)
        if trans_mode: 
            xdot_h[:3] = scaling_trans * np.asarray(axes[:3])
        elif not trans_mode:
            R = np.mat([[1, 0, 0],
                        [0, 1, 0],
                        [0, 0, 1]])
            ...
            axes = np.array(axes[3:])[np.newaxis]
        ...
        curr_time = time.time()
        if curr_time - assist_time >= assist_start and not assist_flag:
