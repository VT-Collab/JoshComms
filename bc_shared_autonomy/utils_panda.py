...
class Joystick(object):
    ...
    def input(self):
        ...
        if abs(dz) < self.deadband:
            dz = 0.0
        A_pressed = self.gamepad.get_button(0) and (
            curr_time - self.lastpress > self.timeband
        )
        B_pressed = self.gamepad.get_button(1) and (
            curr_time - self.lastpress > self.timeband
        )
        START_pressed = self.gamepad.get_button(7) and (
            curr_time - self.lastpress > self.timeband
        )
...
def send2robot(conn, qdot, limit=1.0):
    ...
    send_msg = np.array2string(qdot, precision=5, separator=",", suppress_small=True)[
        1:-1
    ]
    send_msg = "s," + send_msg + ","
    # print(send_msg)
...
def joint2pose(q):
    ...
    def TransX(q, x, y, z):
        return np.array(
            [
                [1, 0, 0, x],
                [0, np.cos(q), -np.sin(q), y],
                [0, np.sin(q), np.cos(q), z],
                [0, 0, 0, 1],
            ]
        )
    ...
    def TransZ(q, x, y, z):
        return np.array(
            [
                [np.cos(q), -np.sin(q), 0, x],
                [np.sin(q), np.cos(q), 0, y],
                [0, 0, 1, z],
                [0, 0, 0, 1],
            ]
        )
