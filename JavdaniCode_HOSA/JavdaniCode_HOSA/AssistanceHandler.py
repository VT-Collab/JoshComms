...
#             'max_alpha': 0.6}  # maximum blending alpha value blended robot policy 
    ...
    def execute_policy(self, direct_teleop_only=False, blend_only=TRUE, fix_magnitude_user_command=TRUE, w_comms = True,confVibe= True,user = 44,demo = 0):
        ...
        filename  = "data/user" + str(user) + "/demo" + str(demo) + ".pkl"
        ...
        while True:
            ...
            if not direct_teleop_only:
                ...
                if auto_or_noto and not direct_teleop_only:
                    SA_time += abs(time.time()-end_time)
            ...
            if STOP:
                ...
                db = {'TotalTime':abs(time.time()-start_time()), 'SA_time' : SA_time,'State':StateList,'UserAction':UserActionList,'AutoAction':AutoActionList}
            ...
            #NOTE TO SELF
            #TOTAL TIME: abs(time.time()-start_time())
            #Shared Autonomy Time = SA_time
            #
