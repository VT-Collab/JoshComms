...
class AssistancePolicyOneGoal:
  ...
  def update(self, robot_state, user_action,max_goal_pos=None,goal_distrib=[],alpha = .4,z = np.zeros([1,6])):
    ...
    for target_policy in self.target_assist_policies:
      target_policy.update(robot_state, user_action,max_goal_pos,goal_distrib,alpha,z)
  ...
  def get_BaseQValue(self):
   # u = self.target_assist_policies[self.min_val_ind].base_Q()
    #print("UUUUUU",u)
  ...
  def get_direct_action(self,goal_pos):

    return self.target_assist_policies[0].get_action(goal_pos) #pick smallest action
