...
class SecondaryPolicy:
	...
	def update(self, robot_state, user_action,goal_distrib = [],alpha = .25):
		...
		for goal_policy in self.goal_assist_policies:
			goal_policy.update(robot_state, self.user_action,self.max_goal.pos,goal_distrib,alpha,self.z)
	...
	def get_assisted_action(self, goal_distribution, fix_magnitude_user_command=False,algo_enabled = False,alpha = 0.1):
		...
		if np.linalg.norm(self.user_action) < .0005:			
			self.user_norm = .03
			self.user_action = Robot_action*.5
			#print("INACTIVE")
		else:
		...
		if np.linalg.norm(Robot_action) < .0005:
			to_ret_twist = UserAdjusted
			blend_curve =  1
		else:
			to_ret_twist = ((1-alpha)*UserAdjusted + alpha*Robot_action) #linear blend
			blend_curve = 2 + (1*alpha)

		if fix_magnitude_user_command:
			to_ret_twist *= ( self.user_norm/np.linalg.norm(to_ret_twist) )*blend_curve
		else:
